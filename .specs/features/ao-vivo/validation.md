...
| Command | Result |
| --- | --- |
| `npm test -- src/data/wedash/live.test.ts` | **PASS** — 6/6 tests, exit 0, ~341ms |
...
| 3. Topbar store selector scopes Ao vivo data | ✅ | `src/layout/Topbar.tsx:36-37,54-56` (SeletorLoja on `/ao-vivo*`); `src/pages/ao-vivo/AoVivoPage.tsx:44,51-53` (`useEscopo` → `buildLiveView(escopo)`). |
...
| AC | Status | Evidence |
| --- | --- | --- |
| 1. Four indicators: Total de Vendas, Faturamento, Meta Mensal, Atingimento | ✅ | Labels defined `src/data/wedash/live.ts:212-233`; rendered `AoVivoPage.tsx:92-107`. Test: `live.test.ts:12-18`. |
| 2. Vendas + Faturamento: month primary, today secondary | ✅ | `live.ts:213-221` — `valor` = month MTD, `sub` = `` `Hoje ${...}` ``. Test: `live.test.ts:15-16`. |
| 3. Meta Mensal = goal amount; Atingimento = % of goal from month revenue | ✅ | `live.ts:205-210,223-232` — `metaAlvo`, `atingimentoPct`, `brlK(mes.faturamento)`. |
| 4. No global Hoje/Mês toggle | ✅ | No toggle/Segmented for period in `src/pages/ao-vivo/*` (grep: no Semestre/Ano/Hoje/Mês toggle). |
| 5. Zero sales → 0 values, no error | ✅ (code) / ⚠️ (test) | Domain uses `num(0)` / `brlK(0)` / `pct(0)` paths (`live.ts:215-216,210-211`). **No dedicated unit test** for zero month/today (see gaps). |
...
| AC | Status | Evidence |
| --- | --- | --- |
| 1. Rank sellers by faturamento desc, month MTD, store scope | ✅ | `live.ts:235-250` — `fatVendedorNoPeriodo(c, mesInicio, mesFim)`, sort `b.ag.faturamento - a.ag.faturamento`. Test: `live.test.ts:21-27`. |
...
| AC | Status | Evidence |
| --- | --- | --- |
| 1. Active challenges as cards (title, rule, time, prize, progress) | ✅ | `blocos.tsx:82-110` — `CardDesafio` with `nome`, `objetivo`, `prazoRotulo`, `premio`, `acumuladoRotulo`, `ProgressBar`. Domain: `live.ts:252-285`. |
| 2. Top 3 participants when challenge has participants | ✅ | `blocos.tsx:99-106` (`d.top3`); built `live.ts:267` (`parts.slice(0, 3)`). |
| 3. No active challenges → empty state | ✅ | `blocos.tsx:113-114`. Test proxy: `live.test.ts:36-40`. |
...
| AC | Status | Evidence |
| --- | --- | --- |
| 1. Competence progress (% and R$ realized / R$ target) | ✅ | `blocos.tsx:136-138`; domain `live.ts:331-336`. |
...
| 4. Por Grupo: groups + nested top sellers | ✅ | `blocos.tsx:190-217` (`g.top3`). Domain `live.ts:305-328`. |
| 5. Goal ladder level legend | ✅ | `blocos.tsx:150-156`; `live.ts:337` (`degrausPadrao`). Test: `live.test.ts:49`. |
| 6. No active goal → empty state | ✅ | `blocos.tsx:127-128`; domain sets `meta: null` when `metaAlvo === 0` (`live.ts:287-288`). |
...
| AC | Status | Evidence |
| --- | --- | --- |
| 1. Evolução section: per-seller monthly table + AreaLineChart | ✅ | `AoVivoPage.tsx:121-123` → `BlocoEvolucao`; table `blocos.tsx:230-254`, chart `blocos.tsx:257-265`. Domain `live.ts:343-355`. Test: `live.test.ts:52-56`. |
| 2. Missing month → em dash (—), chart intact | ✅ | `blocos.tsx:248` (`v == null ? "—"`); chart maps null→0 `blocos.tsx:258-259` without throw. |

**Note:** Chart plots first **2** sellers (`blocos.tsx:257-259,267-271`) while table lists up to **4** (`live.ts:344`). Spec says “per-seller” chart without specifying count — acceptable MVP, minor precision gap.
...
| 2. Click → fixture text, no external API | ✅ | `AoVivoPage.tsx:126` `setInsight(view.insightMock)`; text `live.ts:357-360`. |
...
| --- | --- | --- |
| “Todas as lojas” aggregates KPIs/ranking | ✅ | `live.ts:13-14` (`filialIds.length === 0` → all filiais); test `live.test.ts:30-34`. |
| Store with no sellers → ranking empty | ✅ | `vendedoresNoEscopo` + filter → `ranking.length === 0` → `EmptyState`. |
| Meta exists, realized 0 → Atingimento 0% | ✅ | `live.ts:210` (`mes.faturamento / metaAlvo * 100` → 0). |
| Zero today, month has sales → month KPIs + ranking persist, today sub zeros | ✅ (code) / ⚠️ (test) | Separate aggregates `live.ts:202-203,216-221`; ranking uses month window only. **No unit test** (tasks.md claims coverage). |
...
**Minimal check performed:** `live.test.ts:21-27` asserts descending faturamento order — would fail if `sort` at `live.ts:242` were inverted. Confirms at least one AC-linked invariant is test-guarded.
...
1. **tasks.md vs tests:** T2 “Done when” lists *empty month* and *zero-today-with-month-sales* unit tests — **not present** in `live.test.ts` (only 6 tests). Domain logic appears correct; coverage claim overstated.
