...
16. ✅ **Botão Atualizar (FORCE)** (2026-09-21; **loja 2026-09-22**) — **não** é só “hoje”. Refaz o **período filtrado** (preenche buracos dia a dia) e **sempre inclui hoje**; teto 90 dias; rate limit 1× / 5 min. Escopo = **loja do StorePicker** (`storeIds` no payload); “Todas as lojas” = rede inteira. Sync automático = `LIGHT` a cada **5 min** (`light_interval_min`). FORCE bem-sucedido grava `last_light_sync_at` → **reinicia** a janela do LIGHT (não dispara LIGHT de novo nos mesmos 5 min).
