...
13. ✅ **Progresso da Meta = Σ vendedoras** (2026-09-18) — `metaGlobal.realizado` vem da soma do ranking (não do agregado bruto da loja com caixa). Demo de ritmo só redistribui o bolo real (sem inventar R$). Nível no mês aberto = degrau pelo ritmo/projeção; premiação acumulada só com degrau já cruzado no MTD.
