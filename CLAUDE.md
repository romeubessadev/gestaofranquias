...
41. ✅ **Texto livre GRAVADO e exibido em MAIÚSCULAS** (2026-09-26) — padroniza (usuário digita tudo minúsculo, tudo maiúsculo ou misturado). Vale para **nome de pessoa** (usuário, equipe de vendas, perfil), **nome do turno** e **nome da empresa**. Gravação = `upperText` (`src/lib/format.ts`: trim + espaços únicos + `toLocaleUpperCase("pt-BR")`, mantém acento).
 - Grava em caixa alta: convite (app + Edge `team-members`), turnos (`saveStoreShift`), nome da empresa (`saveTenantBrand` / onboarding), `seller_name` do worker (`sellerDisplayName`), `store_seller.name` (worker e Edge `erp-sellers-sync`, `millenniumSellers.ts` nas duas cópias).
 - Dados antigos normalizados pela migration `20260926150000_upper_free_text` (identity, store_shift, tenant, store_seller, sales_seller_day_agg). A leitura continua passando por `upperText` (rede de segurança).
 - Campo de digitação: `<Input upper>` (Vela `form.tsx`) mostra em maiúsculas via CSS enquanto digita. Campo de texto livre novo → `upper` no campo + `upperText` ao gravar.
