...
32. ✅ **Produtos em dados reais** (2026-09-24) — `buildProductsView(escopo, aggs)` (a fixture saiu). Mesmo padrão do Financeiro e da Visão Geral: filtros = só Período + Exportar (**sem Marca**, sem Atualizar próprio); aviso da carga do mês; sem faixa WPINK (só no Financeiro, 2026-09-26); vazio por card; valores em `brlCent`; recarrega com o Atualizar do Topbar.
 ...
 - **Produto com custo R$ 0 no Millennium** (2026-09-26, decisão do dono): custo zero que **veio** no RELATORIOMARGEM é dado do ERP, não falta de dado → conta como R$ 0 (faixa WPINK deixa de mostrar "—" quando o dia tem margem gravada em `sales_product_cost_day_agg`). Financeiro e Produtos mostram aviso amarelo de 1 linha (ícone de alerta) entre os filtros e os KPIs: "N produtos sem custo no Millennium · CMV e margem ficam acima do real" + "Ver produtos" + **"Recarregar custos"** (OWNER/MANAGER; só com dia passado afetado — hoje o Atualizar cobre). Recarregar = Edge `erp-sync-enqueue` ação `reload` → CLOSE com `{ from: to, to, fillUntil: from, storeIds }` = carga em período (#33) só dos dias/lojas com produto sem custo no período da tela (Lista + cupom + margem 1× por loja por mês; teto 93 dias; outra carga aberta → `reload_busy`). A continuação da carga para meses anteriores mantém as mesmas lojas (`enqueueMonthFillDay` com `storeIds`). Progresso no aviso "Carregando o histórico de vendas"; botão fica "Recarregando…" até a carga terminar. O custo do dia vem do RELATORIOMARGEM de cada dia (não do catálogo) → dias já gravados **não** se corrigem sozinhos; só pelo botão (ou `force-day.ts`) (código · nome do catálogo · itens · R$) — CMV/lucro/margem ficam acima do real até cadastrar o custo no ERP e recarregar os dias. Caso real: WP ULTRA (`WP014`) na 00205 (12/09 = única venda WPINK do dia) e ~24 códigos em ago–set, a maioria na 00114. `ProductsWithoutCostNotice` + `FinanceView.produtosSemCusto`.
 - **Tabela de custo da loja** (2026-09-26, decisão do dono; migration `20260926200000_product_cost_table`) — o custo da margem **não é histórico** (re-buscar dia antigo devolve o custo de hoje) e o zero persiste no ERP, então o "Recarregar custos" sozinho não resolve. As tabelas de custo do Millennium (lookup `tabela_custo.TABELA` + report `{9701602B}` com `TABELA_DE_CUSTO`, filial vazia = tabela inteira, ~3s) têm o preço desses produtos.
 ...
 - **Worker** (`costTableSync.ts`, varredura de 10 min): recarrega a cada **12h** (6h enquanto alguma loja espera detecção), só as tabelas em uso (todas quando precisa detectar). **Nunca faz login** — usa a sessão salva de uma credencial VALID, não pausada e sem job RUNNING; sem sessão = pula. Lease global igual ao catálogo.
 - **Detecção automática**: loja sem tabela escolhida (`cost_table_set_at` null) recebe a tabela com mais custos unitários iguais (±1 centavo) ao último custo da margem por produto (mín. 20 produtos e 50% dos comparados). Santana: 00010/00114/00205 → **104 (Centro-Oeste)**. Loja sem margem com custo (00386) fica sem tabela até ter.
 - **Troca manual**: Configurações > Lojas > loja > Custos da operação > **Tabela de custo** (Select; salva com o card; gravar marca `cost_table_set_at` e a detecção não mexe mais).
 ...
 - Validado: 154 de 162 linhas sem custo (ago–set) preenchidas; segue no aviso só o `504` (zero em todas as tabelas; 48,37 na margem da 00010 em outros dias). "Recarregar custos" continua para quando o ERP for corrigido.
