...
13. ✅ **Progresso da Meta = Σ vendedoras** (2026-09-18) — `metaGlobal.realizado` vem da soma do ranking (não do agregado bruto da loja com caixa). Demo de ritmo só redistribui o bolo real (sem inventar R$). Nível/degrau na UI = MTD (o que já garantiu); projeção alimenta só premiação projetada. Mock set/26: Shopping CG (f1) no Hiper (N3 ≥150%) via `tendencia` 3.25; vendedoras espalhadas N1–N4.
