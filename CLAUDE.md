...
41. ✅ **Texto livre digitado é exibido em MAIÚSCULAS** (2026-09-26) — padroniza a exibição (usuário digita tudo minúsculo, tudo maiúsculo ou misturado). Vale para **nome de pessoa** (usuário, equipe de vendas, perfil), **nome do turno** e **nome da empresa**. **Grava como digitado (ou como veio do ERP), só com `trim`**; a caixa alta é só na exibição (`upperText` em `src/lib/format.ts`: trim + espaços únicos + `toLocaleUpperCase("pt-BR")`, mantém acento).
 - Campo de digitação: `<Input upper>` (Vela `form.tsx`) mostra em maiúsculas via CSS; o valor continua como digitado. Usado em Nome do convite, Nome do turno e Nome da empresa (onboarding). Campo de texto livre novo → usar `upper` + `trim` ao gravar + `upperText` ao exibir.
 - **Não** vale para e-mail, senha, usuário ERP (diferenciam maiúsculas), busca e números.
 - `upperText` aplicado na leitura (sessão, lista de usuários, info do convite, equipe da loja, ranking de vendas, turnos, fixtures) ou direto no JSX (nome da empresa no Topbar/Sidebar) — nunca antes de gravar.
 - Worker grava `seller_name` como veio da Lista (`sellerDisplayName` = trim). Edge `team-members` grava o nome do convite com trim; `{{ .Data.name }}` e `{{ .Data.company }}` do e-mail vão em maiúsculas (é exibição).
