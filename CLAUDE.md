...
41. ✅ **Nome de pessoa sempre em MAIÚSCULAS** (2026-09-26) — usuário do sistema, equipe de vendas e perfil. A pessoa pode digitar minúsculo; gravamos e mostramos em caixa alta (`personName` em `src/lib/format.ts`: trim + espaços únicos + `toLocaleUpperCase("pt-BR")`, mantém acento).
 - Normalizado na entrada dos dados (sessão, lista de usuários, convite, equipe da loja, ranking de vendas) → dado antigo em Title Case aparece em maiúsculas sem migration.
 - Worker grava `seller_name` em maiúsculas (`sellerDisplayName` usa `personName`); Edge `team-members` grava o nome do convite e o do e-mail em maiúsculas.
 - Campo Nome do convite já converte enquanto digita.
