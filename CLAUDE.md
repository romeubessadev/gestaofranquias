...
41. ✅ **Nome de pessoa exibido em MAIÚSCULAS** (2026-09-26) — usuário do sistema, equipe de vendas e perfil. **Grava como a pessoa digitou (ou como veio do ERP), só com `trim`**; a caixa alta é só na exibição (`personName` em `src/lib/format.ts`: trim + espaços únicos + `toLocaleUpperCase("pt-BR")`, mantém acento).
 - `personName` aplicado na leitura (sessão, lista de usuários, info do convite, equipe da loja, ranking de vendas, fixtures) — nunca antes de gravar.
 - Worker grava `seller_name` como veio da Lista (`sellerDisplayName` = trim). Edge `team-members` grava o nome do convite com trim; o `{{ .Data.name }}` do e-mail vai em maiúsculas (é exibição).
 - Campo Nome do convite mostra em maiúsculas via CSS (`uppercase`), valor fica como digitado.
